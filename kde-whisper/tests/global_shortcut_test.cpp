...
private slots:
    ...
    void conditionalApplyRejectsLastMomentExternalChangeWithoutWrite();
};
...
void GlobalShortcutTest::conditionalApplyRejectsLastMomentExternalChangeWithoutWrite()
{
    const QKeySequence baseline(QStringLiteral("Meta+D"));
    const QKeySequence requested(QStringLiteral("Ctrl+Shift+D"));
    const QKeySequence external(QStringLiteral("Meta+Shift+D"));
    QObject owner;
    auto backend = std::make_unique<FakeGlobalShortcutBackend>();
    auto *fake = backend.get();
    fake->savedChoiceExists = true;
    fake->savedChoice = {baseline};
    GlobalShortcutManager manager(&owner, std::move(backend));

    // Model an external KDE change after Settings captured its baseline but on
    // the manager's final authoritative read immediately before mutation.
    fake->changeExternallyBeforeNextRead(external);
    QString error;
    QVERIFY(!manager.applyShortcutIfCurrent(requested, baseline, &error));

    QVERIFY(error.contains(QStringLiteral("changed in KDE")));
    QVERIFY(error.contains(QStringLiteral("not applied")));
    QCOMPARE(manager.currentShortcut(), external);
    QCOMPARE(fake->savedChoice, QList<QKeySequence>{external});
    QCOMPARE(fake->userOverrideCalls, 0);
}
